## robust test to find an interesting group
## rcode=1 normal result
##      =0 abnormal result

rtest = function(y, group, single.only = FALSE, robust.var= TRUE, maxiter=5, err.limit=0.01, upreg=TRUE)
{
# constant matrices
...
# starting estim
  ...
  b = solve(t(X0)%*%X0, t(X0)%*%m)
    if (sum(b <0.1)==ng) return(list(rcode=0, b=b))  ## NOTE: no need to continue
  err= exp(25)
...
## IWLS
 ...
 while (max(err)> err.limit & iter<= maxiter){
   ...
   XWX = t(X) %*% (X*w)
   XWy = t(X)%*% (y*w)
   if (abs(det(XWX))<=1e-20) { XWX = XWX + diag(1e-16,ng) } # if XWX is singular or computationally singular, 
                                                            # add a small value on diagonal to make it invertible - Tian-18062019
   b = solve(XWX, XWy,tol = 1e-20) # use a smaller tolerance - Tian-18062019
   err= (oldb-b)/oldb
   # print(c(b))
  }  # for iter
 ...
 ## robust variance formula (see page 394 of In All Likelihood)
  ...
  } else { cat(paste0("i = ",i,". Caught an error, the estimated variance is negative. Try non robust variance.\n"))
      se = sqrt(diag(XWXi))[-1]  
      se5 = sqrt(rep(1,n4)%*%XWXi[-1,-1]%*%rep(1,n4))
...
if (upreg)  {imax = which((stat)==max((stat)) )  ## looking only on over-expression  
 } else  {imax = which((abs(stat))==max((abs(stat))) )}  ## looking for over and low-expression

if (length(imax) >1) return(list(rcode=0, b=b))
...
if(single.only){   
   ## collect output
      out= c(maxgroup=imax,tstat=stat[imax])
 ...
 #Tian-18062019
 if (abs(det(cvar))<=1e-16) { cvar = cvar+ diag(1e-16,ncol(cvar)) } 
 ...
 ## collect output
 out= c(maxgroup=imax,tstat=stat[imax],chisqstat=cstat)
